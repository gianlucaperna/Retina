...
def pcap_to_csv(dict_param):  # source_pcap, used_port
    ...
    try:
        ...
        start = time.time()
        o, e = subprocess.Popen(command, encoding='utf-8', stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                shell=True).communicate()
        end = time.time()
        ...
        dict_flow_data = {x: gb.get_group(x) for x in gb.groups if
                          x is not None and np.max(gb.get_group(x)["timestamps"]) - np.min(
                              gb.get_group(x)["timestamps"]) > time_drop}
        ...
        if plot == "static":
            ...
            with open(os.path.join(pcap_path,f"{name}.pickle", "wb")) as f:
