    ...
    # Enrich RTP streams with data from RTP codec, RTP tracks
        for key,value in RTP_streams.items():
            ...
            if "ssrc" in value.keys():
                value["ssrc_hex"] = pd.Series(data=stream_to_ssrc[key], index=RTP_streams[key]["ssrc"].index).rename("ssrc_hex")
            ...
            if "codecId" in RTP_streams_short[key].keys():
                ...
                value["codec"] = pd.Series(data= RTP_streams_short[key]["[codec]"], index=RTP_streams[key]["ssrc"].index).rename("codec")
                pc = stream_to_pc[key]
                codecId = RTP_streams_short[key]["codecId"]
                value["payloadType"] = pd.Series(data=RTP_codecs[pc][codecId]["payloadType"], index=RTP_streams[key]["ssrc"].index).rename("payloadType")
                value["mimeType"] =  pd.Series(data=RTP_codecs[pc][codecId]["mimeType"], index=RTP_streams[key]["ssrc"].index).rename("mimeType")
                value["clockRate"] = pd.Series(data=RTP_codecs[pc][codecId]["clockRate"], index=RTP_streams[key]["ssrc"].index).rename("clockRate")
            ...
            if stream_to_track[key]:
                ...
                if "concealmentEvents" in RTP_tracks[track].keys():
                    ...
                    value["concealment_diff"] = RTP_tracks[track]["concealmentEvents"].diff().fillna(-1).rename("concealment_diff")
