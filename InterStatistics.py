...
def inter_statistic (dict_flow_data, LEN_DROP):
    try:
        for flow_id in dict_flow_data:
            ...
            dict_flow_data[flow_id]["inter_time_sequence"] =  (dict_flow_data[flow_id]["rtp_timestamp"]%pow(2,16)) -dict_flow_data[flow_id]["rtp_seq_num"]
            indexNames = dict_flow_data[flow_id][ dict_flow_data[flow_id]['interarrival'] > 1 ].index
            # Delete these row indexes from dataFrame
            LEN_DROP = LEN_DROP + len(indexNames)
            dict_flow_data[flow_id].drop(indexNames , inplace=True) #drop tutti i pacchetti che hanno interarrivo maggiore di un secondo
